...
export function CaptchaModal({ open, onClose, onConfirm, sending, turnstileToken, captchaStatus }: CaptchaModalProps) {
  useEffect(() => {
    if (!open) return
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-2xl">
        <h3 id="captcha-title" className="text-lg font-semibold text-slate-800 dark:text-slate-100">
          Verify and confirm
        </h3>
