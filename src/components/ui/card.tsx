...
const cardVariants = cva('card-hover rounded-2xl border bg-white dark:bg-slate-800 transition-all duration-200', {
  variants: {
    variant: {
...
export function Card({ as: Tag = 'div', className, variant, href, ...props }: CardProps) {
  return <Tag className={cn(cardVariants({ variant, className }))} {...(Tag === 'a' ? { href } : {})} {...props} />
}
